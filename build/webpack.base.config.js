...
if (wee.purgeCss.enabled) {
    let contentPaths = [];
    // const pathOptions = wee.purgeCss.options.paths || wee.purgeCss.paths;
    const pathOptions = (wee.purgeCss.options && wee.purgeCss.options.paths)
        ? wee.purgeCss.options.paths : wee.purgeCss.paths;

    if (Array.isArray(pathOptions)) {
        pathOptions.forEach((pattern) => {
            contentPaths = [
        ...
        contentPaths = [
            ...glob.sync(pathOptions, { nodir: true })
        ];
    }

    config.plugin('purge-css')
        .use(PurgecssPlugin, [{
            ...wee.purgeCss.options,
            paths: contentPaths,
        }])
        .end();
}
