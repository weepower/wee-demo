...
let ids = new Set();
let products = createBaseProducts();
...
function randomSizes() {
	...
	while (results.length < 3) {
		let newSize = faker.random.arrayElement(sizes);
...
function generateTags() {
	...
	if (Math.random() >= 0.5) {
		let random = faker.random.arrayElement(possibleTags);
...
function getImage(product) {
	let type = product.category.slug.replace(/s$/, '');
	let images = [];

	product.colors.forEach((color) => images.push(`/assets/images/products/${type}-${color.title}.jpg`));
	...
	genders.forEach((gender) => {
		shirtVariants.forEach((variant) => {
			shirtTypes.forEach((type) => {
				let title = `${gender} ${variant} ${type} Shirt`;
